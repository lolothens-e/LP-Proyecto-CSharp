    ...
    if isinstance(expr, tuple):
        ...
        if operador in ['+', '-', '*', '/']:
            ...
            if tipo1 == 'bool' or tipo2 == 'bool':
                errores_semanticos.append(f"Error: No se pueden usar booleanos en operaciones aritméticas ({tipo1} {operador} {tipo2})")
            ...
            if tipo1 == tipo2:
                ...
                errores_semanticos.append(f"Tipos incompatibles en operación aritmética: {tipo1} {operador} {tipo2}")
...
def verificar_asignacion(nodo):
    if nodo[0] == 'declaracion_asignacion':
        ...
        if tipo_destino == 'int' and tipo_valor == 'float':
            errores_semanticos.append(f"Error: No se puede asignar un float a un int sin casting explícito")

        if tipo_destino == 'float' and tipo_valor == 'int':
            return  # Permitido implícitamente

        errores_semanticos.append(f"Error: Asignación incompatible de {tipo_valor} a {tipo_destino}")
...
def p_error(p):
    if p:
        ...
        errores_semanticos.append("Error: Se esperaba más código, pero se encontró el final del archivo (EOF).")
...
def p_enter_class_scope(p):
    ...
    if nombre_clas in symbol_table:
        ...
        symbol_table[nombre_clas] = {'type': 'class'}
...
def p_definicion_funcion(p):
    'definicion_funcion : modificador_acceso tipo_retorno IDENTIFICADOR IPAREN DPAREN enter_function_scope bloque_codigo'
    semantic_context['funcion_actual'] = p[3] 
    p[0] = ('def_funcion', p[2], p[3], p[7]) 
